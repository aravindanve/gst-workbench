...
mkdir -p ../outputs/12-hls-abr/240p/
mkdir -p ../outputs/12-hls-abr/360p/
mkdir -p ../outputs/12-hls-abr/480p/
mkdir -p ../outputs/12-hls-abr/720p/
mkdir -p ../outputs/12-hls-abr/1080p/

# create master playlist
cat << EOF >> ../outputs/12-hls-abr/master.m3u8
#EXTM3U
#EXT-X-VERSION:3
...
720p/playlist.m3u8
#EXT-X-STREAM-INF:BANDWIDTH=10500000,RESOLUTION=1920x1080
1080p/playlist.m3u8
EOF
...
GST_DEBUG_DUMP_DOT_DIR=../dots gst-launch-1.0 -v \
    ...
    ! hlssink \
        ...
        max-files=16 \
        playlist-length=8 \
        playlist-location=../outputs/12-hls-abr/160p/playlist.m3u8 \
        target-duration=2 \
    mpegtsmux name=muxer240 `# mpegtsmux or hlssink does not work properly video is not sink_0` \
    ! hlssink \
        location=../outputs/12-hls-abr/240p/chunk%05d.ts \
        max-files=16 \
        playlist-length=8 \
        playlist-location=../outputs/12-hls-abr/240p/playlist.m3u8 \
        target-duration=2 \
    mpegtsmux name=muxer360 `# mpegtsmux or hlssink does not work properly video is not sink_0` \
    ! hlssink \
        location=../outputs/12-hls-abr/360p/chunk%05d.ts \
        max-files=16 \
        playlist-length=8 \
        playlist-location=../outputs/12-hls-abr/360p/playlist.m3u8 \
        target-duration=2 \
    mpegtsmux name=muxer480 `# mpegtsmux or hlssink does not work properly video is not sink_0` \
    ! hlssink \
        location=../outputs/12-hls-abr/480p/chunk%05d.ts \
        max-files=16 \
        playlist-length=8 \
        playlist-location=../outputs/12-hls-abr/480p/playlist.m3u8 \
        target-duration=2 \
    mpegtsmux name=muxer720 `# mpegtsmux or hlssink does not work properly video is not sink_0` \
    ! hlssink \
        location=../outputs/12-hls-abr/720p/chunk%05d.ts \
        max-files=16 \
        playlist-length=8 \
        playlist-location=../outputs/12-hls-abr/720p/playlist.m3u8 \
        target-duration=2 \
    mpegtsmux name=muxer1080 `# mpegtsmux or hlssink does not work properly video is not sink_0` \
    ! hlssink \
        location=../outputs/12-hls-abr/1080p/chunk%05d.ts \
        max-files=16 \
        playlist-length=8 \
        playlist-location=../outputs/12-hls-abr/1080p/playlist.m3u8 \
        target-duration=2 \
    ...
    ! video/x-h264,profile=high \
    ! muxer720. \
    videoscale name=vscaler1080 \
    ! video/x-raw,width=1920,height=1080 \
    ! x264enc \
        speed-preset=fast \
        vbv-buf-capacity=4800 \
        bitrate=2400 \
    ! video/x-h264,profile=high \
    ! muxer1080. \

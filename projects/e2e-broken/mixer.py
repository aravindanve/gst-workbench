...
def do_mixer_stream_init(mixer, **kwargs):
    ...
    stream.rtpdecodebin.locked_state = True
...
def do_mixer_stream_dispose(mixer, stream):
    with mixer.lock:
        # stream.rtpsrcbin.get_static_pad('rtp_src').set_active(False)
        # stream.rtpsrcbin.get_static_pad('rtcp_src').set_active(False)
    ...
    _do_dispose_stream_elements(stream)
    # GLib.idle_add(_do_dispose_stream_elements, stream)
...
def _do_mtunsafe_unlink_rtpbin_from_rtpdecodebin(stream):
    rtpdecodebin_sinkpad = stream.rtpdecodebin.get_static_pad('sink')
    rtpdecodebin_sinkpad.set_active(False)

    if rtpdecodebin_sinkpad.is_linked():
        peerpad = rtpdecodebin_sinkpad.get_peer()
        # peerpad.set_active(False) # NOTE: blocks indefinitely
        peerpad.unlink(rtpdecodebin_sinkpad)
...
def _do_mtunsafe_resize_video_streams(mixer):
    ...
    if mixer.last_resize_video_streams_count != video_streams_count:
        ...
        mixer.last_resize_video_streams_count = index

def _do_dispose_stream_elements(stream):
    print('...................................................(stream_dispose_%s) 1' % stream.session)
    stream.rtpsrcbin.set_state(Gst.State.NULL)
    print('...................................................(stream_dispose_%s) 1.1' % stream.session)
    # mixer.pipeline.remove(stream.rtpsrcbin)

    print('...................................................(stream_dispose_%s) 2' % stream.session)
    stream.rtcpsink.set_state(Gst.State.NULL)
    print('...................................................(stream_dispose_%s) 2.1' % stream.session)
    # mixer.pipeline.remove(stream.rtcpsink)

    print('...................................................(stream_dispose_%s) 3' % stream.session)
    if stream.rtpdecodebin.locked_state:
        stream.rtpdecodebin.set_locked_state(False)

    print('...................................................(stream_dispose_%s) 3.1' % stream.session)
    stream.rtpdecodebin.set_state(Gst.State.NULL)
    print('...................................................(stream_dispose_%s) 3.2' % stream.session)
    # mixer.pipeline.remove(stream.rtpdecodebin)

    print('...................................................(stream_dispose_%s) 4' % stream.session)
...
if __name__ == '__main__':
    ...
    # Timer(15, do_mixer_stream_init_2).start()
    # Timer(15, do_mixer_stream_init_3).start()
    # Timer(25, do_mixer_stream_dispose_2).start()
    # Timer(25, do_mixer_stream_dispose_3).start()
    # Timer(30, do_mixer_stream_init_0).start()
    # Timer(30, do_mixer_stream_init_1).start()
    # Timer(40, do_mixer_stream_init_2).start()
    # Timer(40, do_mixer_stream_init_3).start()
    # Timer(50, do_mixer_stream_dispose_0).start()
    # Timer(50, do_mixer_stream_dispose_1).start()
    # Timer(60, do_mixer_stream_dispose_2).start()
    # Timer(60, do_mixer_stream_dispose_3).start()
    # Timer(70, do_mixer_dispose, args=[mixer]).start()


    Timer(0, do_mixer_start, args=[mixer]).start()
    Timer(2, do_mixer_stream_init_0).start()
    Timer(2, do_mixer_stream_init_1).start()
    Timer(10, do_mixer_stream_init_2).start()
    Timer(10, do_mixer_stream_init_3).start()
    Timer(20, do_mixer_stream_dispose_0).start()
    Timer(20, do_mixer_stream_dispose_1).start()
    Timer(30, do_mixer_stream_dispose_2).start()
    Timer(30, do_mixer_stream_dispose_3).start()
    Timer(40, do_mixer_dispose, args=[mixer]).start()
